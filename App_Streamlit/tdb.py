...
def main() :
    ...
    def percent(table,col):
        df=table.groupby(by=["TARGET",col]).agg({'SK_ID_CURR': 'count'}).reset_index()
        df['Percentage'] = 0
...
# ##########################################################################################################        
    ...
    if sidebar_selection == 'Comparaison des clients': 
        ...
        affichage_graph = st.sidebar.selectbox(label=" Veuillez choisir une catégorie : ",
                                     options=(' ','Genre', "Niveau d'éducation","Types de revenus"))
        ...
        df_nn=df_nn_target.copy()
        df_nn.index=df_nn['SK_ID_CURR']
        X = df_nn.copy()
        X.drop(columns=['TARGET'],inplace=True)
        x_new = X[X.index == identifiant]
        sample=df_nn.copy()
        samples = X.drop(identifiant)
        ...
        clt_sim = df_nn.loc[df_nn.index[index_list]]
        tab=clt_sim[['SK_ID_CURR']]
        tab.index.names =['']
        ...
        html_temp7 = """
            ...
            """ 
        genre_clt = df_compa[df_compa['SK_ID_CURR'] == identifiant]['CODE_GENDER'].iloc[0] 
        clients_sim=pd.merge(tab,df_compa,how='left',on='SK_ID_CURR')
        genre_sim=percent(clients_sim,'CODE_GENDER')
        #client=df_compa[df_compa['SK_ID_CURR'] == identifiant]
        
        educ_clt = df_compa[df_compa['SK_ID_CURR'] == identifiant]['NAME_EDUCATION_TYPE'].iloc[0]
        educ_sim=percent(clients_sim,'NAME_EDUCATION_TYPE')
        educ_sim.dropna(inplace=True)
        
        rev_clt = df_compa[df_compa['SK_ID_CURR'] == identifiant]['NAME_INCOME_TYPE'].iloc[0]
        rev_sim=percent(clients_sim,'NAME_INCOME_TYPE')
        rev_sim.dropna(inplace=True)
        ...
        if affichage_graph=="Niveau d'éducation":
            ...
            st.markdown(html_temp6 + educ_clt, unsafe_allow_html=True)
            bar_plot2(educ_sim, 'NAME_EDUCATION_TYPE')
            bar_plot(education, 'NAME_EDUCATION_TYPE')
            
        if affichage_graph=="Types de revenus":
            
            st.markdown(html_temp7 + rev_clt, unsafe_allow_html=True)
            bar_plot2(rev_sim, 'NAME_INCOME_TYPE')   
